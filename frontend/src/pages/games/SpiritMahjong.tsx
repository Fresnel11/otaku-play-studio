...
import { ArrowLeft, RotateCcw, Ghost, Sparkles, Skull, Shuffle } from 'lucide-react';
...
const SpiritMahjong: React.FC = () => {
    ...
    useEffect(() => {
        if (gameState === 'playing' && tiles.length > 0) {
            ...
            if (visibleTiles.length > 0 && !hasAvailableMoves(visibleTiles)) {
                setMessage("Plus de mouvements ! Mélange des esprits...");
                setTimeout(() => {
                    shuffleBoard();
                    setMessage(null);
                }, 2000);
    ...
    // SIMPLIFIED ATTACK TIMER - This is the key fix
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setAttackTimer(current => {
                const next = current + 1.25; // Increase by 1.25% every 100ms = 8 seconds total

                // When timer reaches 100, trigger attack
                if (next >= 100) {
                    // Attack the player
                    setPlayerHP(hp => {
    ...
    const handleGameEnd = async (result: 'won' | 'lost') => {
        ...
        if (sessionId && userId) {
            ...
            try {
                const finalScore = result === 'won' ? score + 1000 : score;
                const response = await submitGame(sessionId, userId, finalScore, []);
    ...
    const hasAvailableMoves = (currentTiles: Tile[]) => {
        ...
        for (const tile of freeTiles) {
            if (typeCounts[tile.type]) return true;
    ...
    const shuffleBoard = () => {
        setTiles(prev => {
            ...
            const shuffledVisible = visibleTiles.map((t, i) => ({ ...t, type: types[i] }));
    ...
    const handleTileClick = (tile: Tile, e: React.MouseEvent) => {
        ...
        if (selectedTile) {
            if (selectedTile.type === tile.type) {
                ...
                const id1 = Date.now();
                setParticles(prev => [...prev, { id: id1, x: e.clientX, y: e.clientY, type: 'match' }]);
                setTimeout(() => setParticles(prev => prev.filter(p => p.id !== id1)), 1000);

                const id2 = Date.now() + 1;
                setParticles(prev => [...prev, { id: id2, x: window.innerWidth / 2, y: 100, type: 'damage' }]);
                setTimeout(() => setParticles(prev => prev.filter(p => p.id !== id2)), 1000);
            } else {
        ...
        <div className="min-h-screen bg-[#050510] text-white font-sans overflow-hidden relative flex flex-col items-center justify-center">
            ...
            <div className="absolute top-20 left-0 w-full z-40 px-4 md:px-20 flex justify-between items-center">
                ...
                <div className="flex flex-col items-center w-1/3">
                    <div className="w-16 h-16 bg-red-900/20 rounded-full flex items-center justify-center border-2 border-red-500/50 mb-2">
                        <Ghost className="w-8 h-8 text-red-500 animate-pulse" />
                    </div>
                    <div className="w-32 h-2 bg-gray-900 rounded-full overflow-hidden">
                        <motion.div
                            className="h-full bg-red-600"
                            style={{ width: `${attackTimer}%` }}
                        />
                    </div>
